...
- **Editor Scorecard** (rolls up all in-scope grades): per editor — videos count, pillar points (**Brand 25 / QA 30 / Innovation 15 / Output 15 / Revisions 15**), Avg/Day + Target/Day inputs, composite **/100**, and a rating band (🟢 Excellent ≥90 · 🟡 Solid ≥75 · 🟠 Needs Work ≥60 · 🔴 At Risk <60).
  ...
  - **Recommendation column** (added 2026-07-31, far right of the scorecard): a coaching note per editor generated by `gradeRecommendation(primaryCard, monthCard)`. Written to Elsa's **Logic → Evidence → Utility** framework (what it means → the numbers behind it → what to do next) and the project's human-tone rules (contractions, physical verbs, no em dashes, no negation-flips). Combines the rating band (tone) with the **weakest pillar** (targeted action): an At-Risk editor with 0% Brand gets "Sit with Avy, walk 3 recent misses, get the brand rules crisp in their head." Special cases: **Solid across the board** (weakest pillar ≥ 85% fill) drops the pillar mention and says "Reliable across every pillar. … Hand them a harder brief next cycle"; **missing Avg/Day** flips the pace action to "Set it so the real number can show"; **Innovation as the weakest pillar** routes the fix to production, not the editor ("New ideas at N% is dragging, but that's brief-driven. Check what production is queuing for them.") — editors cut what they're briefed, so a low Innovation score is an input-side signal rather than a personal coaching moment. Uses the current-filter composite when it has data; falls back to the This Month rollup so a filter with no data still gets a useful note. Tone chip (`CURRENT SCOPE` / `THIS MONTH`) colour-matches the rating band.
